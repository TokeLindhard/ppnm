...
public static class Integator{
	...
	public static double CC_var_trans(
			Func<double, double> f,
			double a,
			double b,
			double delta = 0.001,
		...
		Func<double, double> ftran;
		if(a == -1 && b == 1){
			ftran = x => f(Cos(x))*Sin(x);
		} else {
			ftran = x => f( (a+b)/2.0 + (b-a)/2.0*Cos(x))*Sin(x)*(b-a)/2.0;
		}

		return integrate(ftran,0,PI);
	} // afslutter CC_var_trans
} //afslutter Integator
