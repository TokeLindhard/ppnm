...
class main {
	...
	public static void inversCalc(){
		WriteLine("Now I will find the inverse matrix of A");
		int k = 6;

		matrix inversA = new matrix(k,k);
		Random ran = new Random();
		for(int i=0; i<k; i++){
			for(int j=0; j<k; j++){
				inversA[i,j] = ran.Next(15);
			}
		}

		WriteLine("Here is the inverse matrix A:");
		inversA.print();
		qrgs QRA = new qrgs(inversA);
		matrix inversB = QRA.inverse();
		
		WriteLine("This is the inverse matrix B of matrix A:");
		inversB.print();
			
		WriteLine($"A*B is approximately equal to the identity matrix I: {(inversA*inversB).approx(matrix.id(k))}");
	}
